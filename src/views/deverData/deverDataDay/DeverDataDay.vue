<template>
    <div id="deverData" class="wrapper flex flex-s">
        <div class="search-bar flex flex-hw">
            ...
            <div class="seach-bar-item flex flex-sc">
                <p>同步数据日期：</p>
                <div class="item-wrapper">
                    <el-date-picker v-model="value1" :picker-options="pickerOptions0" value-format="yyyy-MM-dd" type="date" placeholder="选择日期">
                    </el-date-picker>
                </div>
            </div>
            <div class="seach-bar-item flex flex-sc">
                <el-button type="primary" @click="updateData()">同步</el-button>
            </div>
        </div>
        <div class="content flex">
            <el-table :data="list" height="''" border style="height:100%;width: 100%" v-on:sort-change="sort" :default-sort="{prop: 'dateTime', order: 'descending'}"
                ...
                <el-table-column prop="dateTime" sortable="dateTime" label="日期" width="180"></el-table-column>
...
<script>
    import { getList, getProductList, updateData } from "@/api/deverData/deverDataDay.js";
    export default {
        data() {
            return {
                productList: [],
